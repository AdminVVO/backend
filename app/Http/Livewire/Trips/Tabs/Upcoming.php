...
class Upcoming extends Component
{
    ...
    public function preLoad()
    {
        $this->reservation = ReservationUser::join('listings', 'reservation_users.listing_id', 'listings.id_listings')
                                        ...
                                        ->whereIn('reservation_users.status', [0, 1, 2, 3])
                                        ->get(['photos', 'date_in', 'date_out', 'title', 'descriptions', 'id_reservation_users', 'id_listings', 'street', 'city', 'state', 'zip_code', 'reservation_users.created_at'])
                                        ->toArray();
    ...
    {
        $this->emitTo('trips.tabs.modals.upcoming', 'refreshDetail', $this->reservation[$key]['id_reservation_users']);
    }
}
