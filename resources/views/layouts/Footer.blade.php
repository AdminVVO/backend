<footer>
	<div class="footer entero">
		<div class="medio">
			<div class="footer-container">
				<div class="footer-logo">
					<img src="{{ URL::asset('assets/img/logo-footer.png') }}" alt="logo">

					<div class="cont__ictxtfootr">
						<div class="footer-icon">
							<p class="_icrmrg"><i class="far fa-envelope"></i> Email: vvoutlet@gmail.com</p>
							<p class="_icrmrg"><i class="fas fa-phone-alt"></i> Phone: +800 965 785</p>
						</div>
						<div class="footer-social">
							<a href="#" class="link__icon-footer"><i class="fab fa-facebook-f"></i></a>
							<a href="#" class="link__icon-footer"><i class="fab fa-instagram"></i></a>
							<a href="#" class="link__icon-footer"><i class="fab fa-twitter"></i></a>
						</div>
					</div>
				</div>

				<div class="footer-links">
					<strong>Support</strong>
					<ul>
						<li><a href="#" class="txtunder__hvr _txt-white16">Help Center</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Safety information</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Cancelation options</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Our COVID-19 Response</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Supporting people with disabilities</a></li>
					</ul>
				</div>

				<div class="footer-links">
					<strong>Hosting</strong>
					<ul>
						<li><a href="#" class="txtunder__hvr _txt-white16">Try hosting</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Protection for Hosts</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Explore hosting resources</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Visit our community forum</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">How to host responsibly</a></li>
					</ul>
				</div>

				<div class="footer-links">
					<strong>About</strong>
					<ul>
						<li><a href="#" class="txtunder__hvr _txt-white16">About</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Newsroom</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Learn about new features</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Careers</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Investors</a></li>
						<li><a href="#" class="txtunder__hvr _txt-white16">Vvoutlet Luxe</a></li>
					</ul>
				</div>
			</div>

			<div class="copyright entero">
				<div class="copyright_content">
					<p>
						&copy; 2022 Vvoutlet
						<span class="ptics">·</span>
						<a href="#" class="txtunder__hvr"><span class="_txt-white16">Privacy</span></a>
						<span class="ptics">·</span>
					</p>

					<p>
						<a href="#" class="txtunder__hvr"><span class="_txt-white16">Terms</span></a>
						<span class="ptics">·</span>
						<a href="#" class="txtunder__hvr"><span class="_txt-white16">Sitemap</span></a>
					</p>
				</div>
			</div>
		</div>
	</div>
</footer>