<div class="content-publish_icon">
    ...
    @if ( Auth::check() )
        @if ( in_array($listingId, $wishlists))
            <button type="button" class="_dmptwffb favorite_icon icon_solid" wire:click="$emitTo('home.wishlists', 'removeListing', '{{ $listingId }}')">
        @else
            <button type="button" class="_dmptwffb favorite_icon showFavorite" wire:click="$emitTo('home.wishlists', 'addListing', '{{ $listingId }}')">
        @endif
    @else
        <button type="button" class="_dmptwffb favorite_icon butnSignModl">
    @endif
